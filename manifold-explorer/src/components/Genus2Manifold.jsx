...
import { rotate3D, project3D, parametricGenus2, generateWireframe, wireframeToPath, clamp } from '../mathUtils';
...
export default function Genus2Manifold({ dot, onDotPlace, rotation, onRotationChange, width, height, onTeachingText, path = [] }) {
  ...
  useEffect(() => {
    if (!dot) onTeachingText('A surface with two holes \u2014 genus 2. It needs at least 5 charts to cover it, but we show the parameter space.');
  }, [dot, onTeachingText]);

  const wireframe = useMemo(() => generateWireframe((t, s) => parametricGenus2(t, s), U_LINES, V_LINES, 0, TWO_PI, 0, TWO_PI), []);

  const gridPoints = useMemo(() => {
    const pts = [];
    for (let i = 0; i <= 24; i++) for (let j = 0; j <= 12; j++) pts.push({ u: TWO_PI * (i / 24), v: TWO_PI * (j / 12) });
    return pts;
  }, []);
  ...
  const onClick3D = useCallback((e) => {
    ...
    for (const gp of gridPoints) {
      const pt3d = parametricGenus2(gp.u, gp.v);
  ...
  const onDotPointerMove = useCallback((e) => {
    ...
    u = ((u % TWO_PI) + TWO_PI) % TWO_PI;
    v = ((v % TWO_PI) + TWO_PI) % TWO_PI;
    if (prevDotRef.current) {
      if (Math.abs(u - prevDotRef.current.u) > PI) onTeachingText('The figure-eight path wraps \u2014 exit one side, enter the other.');
      else if (Math.abs(v - prevDotRef.current.v) > PI) onTeachingText('The tube cross-section wraps too \u2014 both directions are periodic.');
    }
  ...
  const dotOn3D = useMemo(() => {
    if (!dot) return null;
    const pt = parametricGenus2(dot.u, dot.v);
  ...
  return (
    <svg ref={svgRef} width={width} height={height}
      ...
      {/* LEFT PANEL: 3D Genus-2 Surface */}
      <g aria-label="3D genus-2 surface view">
        <circle cx={cx3D} cy={cy3D} r={SCALE * 1.2} fill={COLORS.primary} opacity={WIREFRAME.backgroundFillOpacity} />
        {wireframeElements}
        {trailOn3D}

        {/* Rainbow path dots on surface */}
        {pathDots.map((pd, i) => {
          const pt = parametricGenus2(pd.u, pd.v);
          const [rx, ry, rz] = rotate3D(pt.x, pt.y, pt.z, rotation.x, rotation.y);
          const proj = project3D(rx, ry, rz, PERSPECTIVE_D);
          return <circle key={`pt${i}`} cx={cx3D + proj.x * SCALE} cy={cy3D - proj.y * SCALE} r={2} fill={pd.color} opacity={0.85} />;
        })}
        {path.map((wp, i) => {
          const pt = parametricGenus2(wp.u, wp.v);
          const [rx, ry, rz] = rotate3D(pt.x, pt.y, pt.z, rotation.x, rotation.y);
          const proj = project3D(rx, ry, rz, PERSPECTIVE_D);
        ...
        <text x={cx3D} y={height - 16} textAnchor="middle" fill={COLORS.muted} fontSize={13} fontFamily={FONTS.body} fontWeight={600}>Genus-2 Surface</text>
        {!dot && <text x={cx3D} y={height - 36} textAnchor="middle" fill={COLORS.muted} fontSize={11} fontFamily={FONTS.body} opacity={0.6}>Click on the surface to place a dot</text>}
      </g>
      ...
      <g aria-label="Chart view" onClick={onClickChart}>
        <ChartPanel x={chartX} y={chartY} width={chartW} height={chartH}
          label={`Chart \u2014 (t, s) \u2208 [0, 2\u03C0) \u00D7 [0, 2\u03C0)`} uSteps={U_LINES} vSteps={V_LINES}>
