...
import { rotate3D, project3D, parametricCliffordTorus, generateWireframe, wireframeToPath, normalizeAngle, clamp } from '../mathUtils';
...
export default function FlatTorusManifold({ dot, onDotPlace, rotation, onRotationChange, width, height, onTeachingText, path = [] }) {
  ...
  const gridPoints = useMemo(() => {
    const pts = [];
    for (let i = 0; i <= 32; i++) for (let j = 0; j <= 32; j++) pts.push({ u: TWO_PI * (i / 32), v: TWO_PI * (j / 32) });
    return pts;
  }, []);
  ...
  const onClick3D = useCallback((e) => {
    ...
    for (const gp of gridPoints) {
      const pt3d = parametricCliffordTorus(gp.u, gp.v);
  ...
  const trailOn3D = trail.map((tp, i) => {
    const opacity = Math.max(0, 1 - (now - tp.time) / ANIMATION.trailLifetime) * 0.6;
    if (opacity <= 0) return null;
    const pt = parametricCliffordTorus(tp.u, tp.v);
    const [rx, ry, rz] = rotate3D(pt.x, pt.y, pt.z, rotation.x, rotation.y);
    const proj = project3D(rx, ry, rz, PERSPECTIVE_D);
  ...
  return (
    <svg ref={svgRef} width={width} height={height}
      ...
      {/* LEFT PANEL: 3D Clifford Torus */}
      <g aria-label="3D flat torus view">
        ...
        {path.map((wp, i) => {
          const pt = parametricCliffordTorus(wp.u, wp.v);
          const [rx, ry, rz] = rotate3D(pt.x, pt.y, pt.z, rotation.x, rotation.y);
          const proj = project3D(rx, ry, rz, PERSPECTIVE_D);
